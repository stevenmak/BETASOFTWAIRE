(function ($) {
  ...
  function getParticipant(event, size){
    ...
    $.each(event.participant.split(','), function (index, value) {
      participant += '<a href="#" class="avatar w-'+size+'"><img src="../public/assets/img/a'+value+'.jpg"></a>';
    });
    return participant;
  }

  var init = function(){
    $.ajax('../public/assets/api/fullcalendar.json').done(function(data){
      ...
      sr.reveal('.fc-event', {
        ...
        afterReveal:function (el) {
          ...
          if(!noticed){
            notie.alert({ text: 'Pour "Ajouter Tache" button', position: 'top' });
            noticed = true;
          }
        }
      }, 50);
