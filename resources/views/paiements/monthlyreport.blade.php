...
<div class="row">
    ...
    <div class="col-4 text-right">
        ...
        <div class="text-sm text-fade"><h5>Mois de:{{ strtoupper(Carbon::now()->locale('fr')->monthName) }}</h5></div>
    </div>
</div>
<div class="padding">
    <div class="col-12">
        <h2 class="text-center">RAPPORT DU MOIS DE {{ strtoupper(Carbon::now()->locale('fr')->monthName) }}</h2>
    </div>
</div>
<div class="row py-3">
    <table class="table table-theme v-middle table-hover">
        ...
        <tbody>
            @foreach( $entree=DB::table('paiements')->whereMonth('datepaiement','=',Carbon\Carbon::now()->month)
            ->get() as $key => $value)
                <tr class=" v-middle" data-id="2">
                    ...
                    <td class="flex">
                        <div class="item-title"> {{$projet = DB::table('clients')->join('projets','projets.client_id', '=', 'clients.id')
        ...
        <tfoot>
            <tr>
                <td colspan="6" class="no-wrap">
                    <div class="text-right">Total du mois:  {{$entree=DB::table('paiements')->whereMonth('datepaiement','=',Carbon\Carbon::now()->month)
